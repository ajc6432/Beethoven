...
public final class PitchEngine {
    ...
    public func start() {
        ...
        case .granted:
            activate()
        case .denied:
            delegate?.pitchEngine(self, didReceiveError: PitchEngineError.microphoneAccessDenied)
        case .undetermined:
            delegate?.pitchEngine(self, didReceiveError: PitchEngineError.needMicrophoneAccess)
        @unknown default:
            break
        }
    }
